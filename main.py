...
class xthor(TorrentProvider, MovieProvider):
    ...
    def _searchOnTitle(self, title, movie, quality, results, passkey):
        ...
        log.debug('Looking on Xthor for movie named %s or %s' % (title, newTitle))
        url = self.urls['api'] % (self.conf('passkey'), request)
        log.debug('Request to Xthor: %s' % (url))
        data = self.getHTMLData(url)

        log.debug('Received data from Xthor')
        if data:
            log.debug('Data is valid from Xthor')
			log.debug(data);
            except:
